...
int main(int argc, const char * argv[]) {
    ...
    double  xi, yi, zi;     /* x y en z positie als de beam onder 90 graden staat */
    double  dzx, dzy;       /* dummy variables for caluclating new z */
	double	ux, uy, uz;     /* photon trajectory as cosines */
	double  uxd, uyd, uzd;  /* photon trajectory in degrees */
	double  uxx, uyy, uzz;	/* temporary values used during SPIN */
	double  uzcatch;		/* Temp val for uz trajectory calculation */
	...
	long	reqphotons;     /* requested photons */
	...
	fgets(buf, 32, fid);
		...
		fgets(buf, 32, fid);
	    sscanf(buf, "%d", &reqphotons);  // # of bins  
		fgets(buf, 32,fid);
        ...
        fgets(buf, 32,fid);
		sscanf(buf, "%f", &focal_length);  // focal length
        fgets(buf, 32,fid);
		sscanf(buf, "%f", &angle_x);  // angle_x
		fgets(buf, 32,fid);
		sscanf(buf, "%f", &angle_y);  // angle_y
...
/*     iy = Ny/2;
	...
	do {
		...
		/* if ((i_photon>1000) & (fmod(i_photon, (int)(reqphotons/100))  == 0)) {
            temp = i_photon/reqphotons*100;
            //printf("%0.1f%% \t\tfmod = %0.3f\n", temp,fmod(temp, 10.0));
            if ((temp<10) | (temp>90)){
                printf("%0.0f%% done\n", i_photon/reqphotons*100);
            }
...
//                 printf("r SOURCE %0.4f \n",r);
// 				printf("r radius of beam at launch point %0.4f [cm]\n",r);
				while ((rnd = RandomGen(1,0,NULL)) <= 0.0); // avoids rnd = 0
				phi		= rnd*2.0*PI;
// 				printf("phi SOURCE %0.4f \n",phi);
				/* X COORDINAAT BEPALEN */
                xi		= r*cos(phi);
//                 printf("anglex %0.4f \n",angle_x);
//                 if (angle_x==90) {x=xi; dzx=0;}
                
//                 else {
                dzx     = xi*sin((90-angle_x)*PI/180);
                x       = xs-xi*cos((90-angle_x)*PI/180);
//                 printf("x SOURCE %0.4f \n",x);
//                 printf("test %0.4f \n",cos((90-angle_x)*PI/180));
//                 printf("test2 %0.4f \n",xi*cos((90-angle_x)*PI/180));
				
				
                /* Y COORDINAAT BEPALEN */
                yi		= r*sin(phi);
//                 if (angle_y==90) {y=yi; dzy=0;}
//                 else {
                dzy     = yi*sin((90-angle_y)*PI/180);
                y       = ys-yi*cos((90-angle_y)*PI/180);
                
// 				printf("y SOURCE %0.4f \n",y);
				z		= zs+dzx+dzy;
//                 printf("z SOURCE %0.4f \n",z);
//                 
//              calculating unit direction vector
//                 printf("xfocus %0.4f \n",xfocus);
...
//                 ux=0.23; uy=0.23; uz=0.94;
	...
	} while (i_photon < reqphotons);  /* end RUN */
	
    
	printf("------------------------------------------------------\n");
	finish_time = clock();
	time_min = (double)(finish_time-start_time)/CLOCKS_PER_SEC/60;
	reqphotons=(double)reqphotons;
	printf("Elapsed Time for %d photons = %5.3f min\n",reqphotons,time_min);
	printf("%1.3e photons per minute\n", reqphotons/time_min);
	printf("Performed for requested photons %d \n",reqphotons);
